...
namespace RE
{
	...
	public:
		...
		{
			std::uint32_t crc32Filename;  // 00 - BSCRC32 of lowercase filename without extension
			char          extension[4];   // 04 - plaintext extension without dot (e.g. "hkx")
			std::uint32_t crc32Path;      // 08 - BSCRC32 of lowercase path to the containing directory, starting with "meshes"
		};
