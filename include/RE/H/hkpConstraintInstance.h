...
namespace RE
{
	...
	struct hkConstraintInternal
	{
		...
		std::uint16_t                                                             atomsSize;           // 20
		std::uint8_t                                                              callbackRequest;     // 22
		stl::enumeration<hkpConstraintInstance::ConstraintPriority, std::uint8_t> priority;            // 23
		std::uint16_t                                                             sizeOfSchemas;       // 24
		std::uint16_t                                                             numSolverResults;    // 26
		std::uint16_t                                                             numSolverElemTemps;  // 28
		std::uint8_t                                                              whoIsMaster;         // 2A
		bool                                                                      isNormalType;        // 2B
		hkpConstraintRuntime*                                                     runtime;             // 30
		std::uint16_t                                                             runtimeSize;         // 38
