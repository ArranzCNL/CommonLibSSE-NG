...
namespace RE
{
	namespace BSGraphics
	{
		...
		{
			ClibUtil::Vector4 viewUp;                            // 00
			ClibUtil::Vector4 viewRight;                         // 10
			ClibUtil::Vector4 viewForward;                       // 20
			ClibUtil::Matrix  viewMat;                           // 30
			ClibUtil::Matrix  projMat;                           // 70
			ClibUtil::Matrix  viewProjMat;                       // B0
			ClibUtil::Matrix  unknownMat1;                       // F0 - all 0?
			ClibUtil::Matrix  viewProjMatrixUnjittered;          // 130
			ClibUtil::Matrix  previousViewProjMatrixUnjittered;  // 170
			ClibUtil::Matrix  projMatrixUnjittered;              // 1B0
			ClibUtil::Matrix  unknownMat2;                       // 1F0 - all 0?
...
#define RUNTIME_DATA_CONTENT                                                                                                         \
				std::uint32_t             firstCameraStateIndex;                /*	058 VR   only ?*/                                 \
				...
				std::uint16_t             dynamicResolutionCounter;             /* SE 118, AE,VR 11c */
#else
#define RUNTIME_DATA_CONTENT                                                                                                         \
				NiPointer<NiTexture>      defaultTextureBlack;                  /* SE 058, AE,VR 060 - "BSShader_DefHeightMap"*/      \
				NiPointer<NiTexture>      defaultTextureWhite;                  /* SE 060, AE,VR 068 */                               \
			...
			{
				for (std::uint32_t i = 0; i < GetRuntimeData().cameraDataCacheA.size(); i++) {
